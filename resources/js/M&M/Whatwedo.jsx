...
export default function WhatWeDo() {
	...
	useEffect(() => { // Set initial hidden states
		gsap.set([
		...
		loadTimeline.to(tagRef.current, {
			opacity: 1,
			x: 0,
			duration: 0.6
			...
			x: 0,
			duration: 0.6
		}, "-=0.3").to(textRefs.current, {
			opacity: 1,
			y: 0,
			stagger: 0.2,
			duration: 0.5
		}, "-=0.2").to(ctaRef.current, {
			opacity: 1,
			y: 0,
			scale: 1,
			duration: 0.7,
			ease: "back.out(0.7)"
		}, "-=0.2").to(imageRef.current, {
			opacity: 1,
			scale: 1,
			rotationY: 0,
			duration: 0.9,
			ease: "power2.out"
		}, "-=0.5").to(cardsRef.current, {
			opacity: 1,
			x: 0,
			scale: 1,
			stagger: 0.15,
		...
		cardsRef.current.forEach((card) => {
			...
			card.addEventListener('mouseenter', () => { // Slide overlay up from bottom
				if (overlay) {
					gsap.to(overlay, {
						y: 0,
						duration: 0.4,
				...
				svgPaths.forEach(path => gsap.to(path, {
					stroke: "white",
					duration: 0.3
				}));
			});

			card.addEventListener('mouseleave', () => { // Slide overlay back down
				if (overlay) {
					gsap.to(overlay, {
						y: "100%",
						duration: 0.4,
	...
	return (
		<div ref={sectionRef}
			className="bg-[#222629] flex items-center justify-center overflow-hidden">
			<div className="w-full max-w-7xl mx-auto py-14 md:py-24 px-5 sm:px-6 grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12 items-center relative">
				...
				<div ref={leftColumnRef}
					...
					{/* Tag */}
					<div ref={tagRef}
						className="flex items-center text-white text-sm font-bold tracking-[2px] uppercase mb-4 sm:mb-6">
						WHAT WE DO
					</div>

					{/* Heading */}
					<h2 ref={headingRef}
						className="text-white text-3xl sm:text-4xl md:text-5xl font-bold leading-[1.2] sm:leading-[52px] mb-4 sm:mb-6">
					...
					{/* Body */}
					<p ref={
							el => textRefs.current[0] = el
						}
						className="text-white text-sm sm:text-base leading-[24px] sm:leading-[26px] mb-3 sm:mb-4">
					...
					<p ref={
							el => textRefs.current[1] = el
						}
						className="text-white text-sm sm:text-base leading-[24px] sm:leading-[26px] mb-6 sm:mb-8">
					...
					<div ref={ctaRef}
						...
						<div className="flex flex-col justify-center px-5 sm:px-8 md:pl-10 py-0 flex-1">
							...
							<a href="tel:+97701-4535104" className="text-white font-extrabold text-sm sm:text-base md:text-lg tracking-[0.5px] mt-0.5">
								+977 01-4535104
								</a>
				...
				<div ref={rightColumnRef}
					className="flex justify-center lg:justify-end items-center relative mt-4 lg:mt-0">
