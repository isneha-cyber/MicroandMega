...
const Products = () => {
	...
	useEffect(() => {
		const fetchProducts = async () => {
			try { // Change from route("ourproducts.index") to just the URL
	...
	const handleDelete = async (id) => {
		try {
			const response = await axios.delete(route("ourproducts.destroy", {id: id}),);
	...
	const handleUpdate = async (formData, id) => {
		try {
			...
			const response = await axios.post(route("ourproducts.update", {id}), // Fixed route name
				formData, {
				headers: {
		...
		<>
			<AdminWrapper>
				<div className="container mx-auto py-4">
					...
					{
					showAddForm && (
						<AddProducts editingProducts={editingProducts}
