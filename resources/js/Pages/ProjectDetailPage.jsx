...
export default function ProjectDetailPage() {
    ...
    const seoTitle = project.title
        ? `${project.title} | Micro & Mega`
        : project.name
        ? `${project.name} | Micro & Mega`
        : "Project | Micro & Mega";

    const seoDescription = project.description
        ? project.description.slice(0, 155)
        : "View project details by Micro & Mega Nepal.";
    ...
    return (
        <>
            ...
            {/* ── Hero Banner ──────────────────────────────────────────────── */}
            <div
                className="relative flex min-h-[220px] sm:min-h-[380px] lg:min-h-[500px] items-center justify-center bg-cover bg-center bg-no-repeat px-6 py-12 lg:bg-fixed"
                style={{ backgroundImage: "url('/images/about-bg.jpg')" }}
            >
                <div className="absolute inset-0 bg-gray-900/75 pointer-events-none" />
                <div className="relative z-20 flex flex-col items-center text-center gap-3 max-w-3xl mt-24">
                        ...
                        <div className="flex flex-col gap-8 min-w-0">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                {[
                                    ...
                                    .filter(s => s.value || s.rating)
                                    .map(({ icon: Icon, label, value, rating }) => (
                                        <div key={label} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex items-start gap-3">
                                            <div className="w-8 h-8 bg-red-50 rounded-lg flex items-center justify-center flex-shrink-0">
                                                <Icon size={14} className="text-red-600" />
                                            </div>
