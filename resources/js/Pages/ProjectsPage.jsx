...
function ProjectCard({project, cardRef}) {
    return (<Link href={`/project-details/${project.slug}`}
        ref={cardRef}
        className="group relative flex flex-col bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-xl transition-shadow duration-300 cursor-pointer"
        style={{willChange: "transform, opacity"}}>
        <div className="relative w-full h-52 overflow-hidden">
            <img src={project.image || '/placeholder-image.jpg'}
                alt={project.name}
                className="w-full h-full object-cover transition-transform duration-700 ease-out group-hover:scale-105"/>
            <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent"/>
            <div className={`absolute top-3 left-3 text-[11px] font-bold px-2.5 py-1 rounded-full ${getCategoryColor(project.category)}`}>
                {project.category}
            </div>
            <div className="absolute top-3 right-3 bg-black/50 text-white text-[11px] font-semibold px-2.5 py-1 rounded-full backdrop-blur-sm">
                {project.year}
            ...
            <div className="absolute bottom-3 left-3 right-3 flex items-end justify-between">
                <StarRating rating={project.rating || 4}/>
                <span className="text-white text-[11px] font-medium bg-black/40 px-2 py-0.5 rounded-full backdrop-blur-sm">
                    {project.contractType}
                </span>
            </div>
        ...
        <div className="flex flex-col flex-1 p-5 gap-3">
            ...
            <div className="flex flex-wrap gap-1.5">
                {project.tags && project.tags.slice(0, 3).map((tag) => (
                    <span key={tag} className="text-[10px] font-semibold text-gray-600 bg-gray-100 px-2 py-0.5 rounded-full">
                        {tag}
                    </span>
                ))}
            </div>
...
function getCategoryColor(category) {
    const colors = {
        ...
        'Diplomatic': 'bg-rose-100 text-rose-700'
    };
...
export default function ProjectsPage() {
  ...
  // In ProjectsPage.jsx, update the fetchProjects function
const fetchProjects = async () => {
    try {
        setLoading(true);
        // Try with the full URL without /api prefix if you didn't add it
        const response = await axios.get('/ourprojects?per_page=100');
        // Or if you're using the prefix:
        // const response = await axios.get('/api/ourprojects?per_page=100');
    ...
    useEffect(() => {
        ...
        const cards = cardRefs.current.filter(Boolean);
        gsap.set([headingRef.current, filterRef.current], {
            opacity: 0,
            y: 30
        });
        gsap.set(cards, {
    ...
    return (<>
        ...
        <section ref={sectionRef} className="bg-gray-50 min-h-screen py-14 sm:py-20 px-4 sm:px-6 lg:px-8">
            <div className="max-w-7xl mx-auto flex flex-col gap-10">
                <div className="flex flex-col gap-4">
                    <div ref={headingRef} className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                        <div>
                            <p className="text-red-600 text-sm font-bold tracking-[2px] uppercase mb-1">Portfolio</p>
                            <h2 className="text-3xl sm:text-4xl font-extrabold text-gray-900 leading-tight">
                                Projects We've
                        ...
                        <p className="text-gray-500 text-sm">
                            {filtered.length} project{filtered.length !== 1 ? "s" : ""} — {activeCategory === "All" ? "all sectors" : activeCategory}
                        </p>
                    </div>
                    <div ref={filterRef}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
                    {filtered.map((project, i) => (
                        <ProjectCard key={project.slug} project={project} cardRef={(el) => (cardRefs.current[i] = el)}/>
                    ))}
                </div>
